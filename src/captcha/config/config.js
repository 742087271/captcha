...
class CaptchaConfig {
    constructor(args) {
        ...
        this.requestChain = [];
        this.validData = args.validData || {};
        this.requestMethod = args.requestMethod;
        this.infoText = args.infoText;
        // 时间戳转换
        this.timeToTimestamp = args.timeToTimestamp;
    ...
    doSendRequest(requestParam) {
        ...
        const data = JSON.parse(requestParam.data)

            data.imageCaptchaTrack = data.data
            delete data.data
        return this.requestMethod(requestParam.url,data).then(res => {
            try {
    ...
    validCaptcha(currentCaptchaId, data, c, tac) {
        ...
        return request.then(res => {
            ...
            if (res.code == 200) {
                ...
                c.showTips(this.infoText.success(useTimes), 1, () => this.validSuccess(res, c, tac));
            } else {
                let tipMsg = this.infoText.failed;
                if (res.code) {
                    if (res.code != 4001) {
                        tipMsg = this.infoText.failed;
                    }
                }
